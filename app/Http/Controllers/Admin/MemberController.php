...
class MemberController extends CustomController
{
    ...
    {
        $data = User::with('guru')->where('id', '=', $id)->firstOrFail();
    ...
    public function patch()
    {
        try {
            ...
            $member = Guru::with('user')->where('user_id', '=', $user->id)->firstOrFail();
            $member_data = [
                'nama' => $this->postField('nama'),
                'no_hp' => $this->postField('no_hp'),
                'alamat' => $this->postField('alamat')
    ...
    public function destroy()
    {
        try {
            DB::beginTransaction();
            $id = $this->postField('id');
            Guru::with('user')->where('user_id', '=', $id)->delete();
            User::destroy($id);
            DB::commit();
